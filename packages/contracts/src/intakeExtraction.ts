export function isIntakeUrgent(normalised: string): boolean {
  return /\b(?:today|tonight|urgent|emergency|immediate|stopped|down|line stop|fault)\b/.test(
    normalised
  );
}

export function isIntakeRecoveryRequirement(normalised: string): boolean {
  return (
    isIntakeUrgent(normalised) ||
    /\b(?:alarm|dead|failed|failure|not working|overheating|tripping|diagnose|repair)\b|(?:is not|isn't|isnt)\s+(?:heating|melting|working)/.test(
      normalised
    )
  );
}

export function detectIntakeEquipment(normalised: string): string[] {
  ...
  if (normalised.includes("gearbox")) equipment.add("Industrial gearbox");
  if (/\bmotor\b/.test(normalised)) equipment.add("Industrial motor");
...
export function detectIntakeCapabilities(
  normalised: string,
  ...
  if (normalised.includes("plc")) {
    capabilities.add(
      requiresRecovery ? "PLC fault finding" : "PLC integration"
    );
  }
...
export function detectIntakeLocation(rawRequirement: string): string | undefined {
  const statePattern =
    "(?:[Nn][Ss][Ww]|[Vv][Ii][Cc]|[Qq][Ll][Dd]|[Ss][Aa]|[Ww][Aa]|[Tt][Aa][Ss]|[Nn][Tt]|[Aa][Cc][Tt])";
  const locationWithState = rawRequirement.match(
    new RegExp(
      String.raw`(?:^|[^A-Za-z])((?:[A-Z][A-Za-z'-]*(?:\s+[A-Z][A-Za-z'-]*){0,2})),?\s+(${statePattern})\b`
    )
  );
...
export function detectIntakeUrgency(
  ...
  if (hours) {
    ...
    if (count <= 24) return "Required today";
...
export function detectIntakeBudget(
  rawRequirement: string
): string | undefined {
  ...
  const contextual = rawRequirement.match(
    new RegExp(
      String.raw`\b(?:budget|callout tolerance)\D{0,20}(${numberPattern})`,
      "i"
    )
  );
...
export function intakeCategoryFromEquipment(
  equipmentOrTechnology: string[]
): string {
  if (
    equipmentOrTechnology.some((item) =>
...
export function truncateIntakeTitle(value: string): string {
  ...
  const truncated =
    lastBoundary > 0 ? candidate.slice(0, lastBoundary).trimEnd() : candidate;
  return `${truncated}…`;
}

function formatIntakeAmount(value: string): string {
