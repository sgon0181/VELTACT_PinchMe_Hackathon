...
export const PRE_NEED_INTAKE_DRAFT_KEY =
  "veltact:rapidmatch:pre-need-intake-draft";

export type PreNeedIntakeDraft = {
...
type PersistedPreNeedIntakeDraft = {
  version: 1;
  requirementInput: BuyerRequirementInput;
  priority: PrioritySignal;
  intakeSourceMode: IntakeSourceMode;
...
export function validateIntakeRawRequirement(value: string) {
...
export function intakeRawRequirementGuidance(value: string) {
  const length = value.trim().length;
  if (length === 0) {
...
export function serializePreNeedIntakeDraft(
  draft: PreNeedIntakeDraft
): string {
  const value: PersistedPreNeedIntakeDraft = {
    version: 1,
...
export function parsePreNeedIntakeDraft(
  raw: string | null
): PreNeedIntakeDraft | undefined {
  if (!raw) return undefined;
  try {
    const value = JSON.parse(raw) as Record<string, unknown>;
    if (value.version !== 1) return undefined;
    const requirementInput = parseBuyerRequirementInput(
...
export function parseBuyerRequirementInput(
  value: unknown
): BuyerRequirementInput | undefined {
...
function stripBinaryEvidence(item: IntakeEvidence): IntakeEvidence {
  return {
    kind: item.kind,
    name: item.name,
...
function cloneRequirementInput(
