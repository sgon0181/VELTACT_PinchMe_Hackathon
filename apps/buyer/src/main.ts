...
function bindEvents() {
  ...
  document.querySelector<HTMLButtonElement>("#structure-button")?.addEventListener("click", async () => {
    ...
    await run(async () => {
      ...
      intakeDraft = {
        ...
        equipmentOrTechnology: mergeFieldValues(
          csvValues(formData, "equipmentOrTechnology"),
          structured.equipmentOrTechnology
        ),
        requiredCapabilities: mergeFieldValues(
          csvValues(formData, "requiredCapabilities"),
          structured.requiredCapabilities
        ),
        ...
        constraints: mergeFieldValues(
          csvValues(formData, "constraints"),
          structured.certificationsOrConstraints
        )
      };
  ...
  document.querySelector<HTMLButtonElement>("#review-structured-button")?.addEventListener("click", () => {
    document.querySelector<HTMLElement>("#editable-need-profile")?.scrollIntoView({
      behavior: "smooth",
      block: "start"
    });
  ...
  document.querySelector<HTMLButtonElement>("#refresh-responses-button")?.addEventListener("click", async () => {
    ...
    await run(async () => {
      ...
      if (
        selectedResponseId &&
        !workspace.responses.some(
          (response) => response.id === selectedResponseId && response.decision === "can_help"
        )
      ) {
        selectedResponseId = "";
      }
...
function mergeFieldValues(existing: string[], structured: string[]) {
