...
function renderConnectHandoff() {
  if (!workspace) return "";
  const hasBuyerApproval = workspace.supplierLeads.some(
    (lead) => lead.lifecycleStatus !== "discovered"
  );
  ...
  return `
    <div class="handoff-guide" aria-label="Buyer and supplier handoff">
      ${stages
        .map(
          (stage, index) => `<div class="handoff-step ${stage.complete ? "is-complete" : index === activeIndex ? "is-active" : ""}">
  ...
  try {
    ...
    const selectableLeadIds = new Set(
      workspace.supplierLeads
        .filter((lead) =>
          ["discovered", "approved_for_outreach"].includes(
            lead.lifecycleStatus
          )
        )
        .map((lead) => lead.id)
    ...
    selectedLeadIds = new Set(
      [...selectedLeadIds].filter((leadId) => selectableLeadIds.has(leadId))
    );
