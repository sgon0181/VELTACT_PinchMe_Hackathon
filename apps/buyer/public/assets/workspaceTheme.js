...
export const DEFAULT_WORKSPACE_THEME = "light";
...
export function isWorkspaceTheme(value) {
...
export function readStoredWorkspaceTheme(storage = browserStorage()) {
    try {
...
export function storeWorkspaceTheme(theme, storage = browserStorage()) {
    try {
...
export function applyWorkspaceTheme(theme, root = document.documentElement) {
...
function updateToggleCopy(button, currentTheme) {
    const nextTheme = currentTheme === "light" ? "dark" : "light";
    const label = button.querySelector(".workspace-theme-toggle__label");
    if (label) {
...
export function mountWorkspaceThemeToggle() {
    const existing = document.querySelector("[data-workspace-theme-toggle]");
    ...
    button.addEventListener("click", () => {
        const activeTheme = isWorkspaceTheme(document.documentElement.dataset.workspaceTheme)
...
function initialiseWorkspaceTheme() {
    ...
    window.addEventListener("storage", (event) => {
        if (event.key !== WORKSPACE_THEME_STORAGE_KEY &&
            event.key !== null) {
            return;
        }
        const nextTheme = isWorkspaceTheme(event.newValue)
            ...
            : DEFAULT_WORKSPACE_THEME;
        applyWorkspaceTheme(nextTheme);
        const button = document.querySelector("[data-workspace-theme-toggle]");
        if (button) {
            updateToggleCopy(button, nextTheme);
        }
    });
}
function initialisePointerGlow() {
    ...
    document.addEventListener("pointermove", (event) => {
        ...
        const target = event.target instanceof Element
            ? event.target.closest(WORKSPACE_GLOW_SURFACE_SELECTOR)
            : null;
        if (!target) {
            return;
        }
