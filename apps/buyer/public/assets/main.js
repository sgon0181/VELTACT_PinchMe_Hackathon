...
const roboticsDemoInput = {
    ...
    description: "Our ABB palletising robot stopped mid-cycle on Line 3. The Siemens S7 PLC shows an intermittent safety-circuit fault and cartons are backing up. We need a robotics and automation specialist in Western Sydney tonight to diagnose the cell, restore safe production and document the fix before the 6:00 am supermarket dispatch.",
...
function evidenceStatusLabel(item) {
    if (item.dataUrl)
        return "File content sent to API intake wrapper";
    if (item.extractedText)
        return "Text sent to API intake wrapper";
...
function bindEvents() {
    ...
    document.querySelector("#robotics-demo-button")?.addEventListener("click", () => {
    ...
    document.querySelector("#plc-demo-button")?.addEventListener("click", () => {
...
function loadDemoInput(input, message) {
    intakeDraft = {
        ...input,
...
async function collectIntakeEvidence(form) {
    const formData = new FormData(form);
    const evidence = [];
...
function fileFromForm(form, name) {
    const fieldElement = form.elements.namedItem(name);
...
async function evidenceFromFile(file, fallbackKind) {
    ...
    const kind = file.type.startsWith("image/") ? "photo" : file.type === "application/pdf" || file.name.toLowerCase().endsWith(".pdf") ? "pdf" : fallbackKind;
...
function mobileDeliveryLabel(data) {
    return data.outreachDeliveries.some((delivery) => delivery.channel === "sms" && isWhatsAppDestination(delivery.destination))
        ? "WhatsApp"
        : "SMS";
}
function deliveryChannelLabel(delivery) {
    return isWhatsAppDestination(delivery?.destination) ? "WhatsApp" : "SMS";
}
function isWhatsAppDestination(destination) {
