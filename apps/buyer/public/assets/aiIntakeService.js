...
export class DemoAiIntakeService {
    ...
    async structureRequirement(input) {
        ...
        const requiredCapabilities = detectCapabilities(normalised, equipmentOrTechnology, isUrgent);
        ...
        return aiIntakeResultSchema.parse({
            rawRequirement,
            generatedProfile: {
                title: titleFromRequirement(rawRequirement, equipmentOrTechnology, isUrgent),
...
function detectCapabilities(normalised, equipmentOrTechnology, isUrgent) {
    const capabilities = new Set();
    if (normalised.includes("robot")) {
        capabilities.add(isUrgent ? "Robotic cell fault recovery" : "Robotic systems integration");
    }
    if (normalised.includes("abb")) {
        capabilities.add(isUrgent ? "ABB robot diagnostics" : "ABB robot programming");
    }
    if (/palletis|palletiz|pallet(?:\s+|-)?load/.test(normalised)) {
        capabilities.add(isUrgent ? "Palletising cell recovery" : "Palletising cell integration");
    }
    if (normalised.includes("vision")) {
        capabilities.add("Machine vision integration");
    }
    if (normalised.includes("siemens")) {
        capabilities.add(isUrgent ? "Siemens PLC diagnostics" : "Siemens controls integration");
    }
    if (normalised.includes("plc")) {
        capabilities.add(isUrgent ? "PLC fault finding" : "PLC integration");
    }
    if (normalised.includes("conveyor")) {
        capabilities.add(isUrgent ? "Conveyor fault recovery" : "Conveyor integration");
    }
    if (normalised.includes("safety") ||
        /\bsafe\s*guard|\bsafeguard|\bguarding/.test(normalised)) {
        capabilities.add(isUrgent ? "Safety circuit diagnostics" : "Machinery safety");
    }
...
function titleFromRequirement(rawRequirement, equipmentOrTechnology, isUrgent) {
